...
function notifyServiceWorker(message) {
  ...
  const send = () => navigator.serviceWorker.controller?.postMessage(message);
...
export function clearPrivateFileCache() {
  notifyServiceWorker({ type: 'CLEAR_PRIVATE_CACHE' });
}
...
export const api = {
  ...
  testPushNotification:     (body)     => request('/auth/push-notification', { method: 'POST', body: JSON.stringify(body) }),
  ...
  // GET   /api/upload/route-file/:uid/:filename?name=…  → download
  ...
  routeFileUrl: (userId, filename, name) =>
    `${BASE}/upload/route-file/${userId}/${encodeURIComponent(filename)}?name=${encodeURIComponent(name)}`,
  ...
  // GET   /api/upload/attachment/:uid/:filename[&download=1]  → inline / download
